...
- 生产部署目录为 `<DEPLOY_DIR>`，持久化上传和日志目录位于 `<APP_DATA_ROOT>`；该目录是运行产物目录，不是 Git 工作区。
...
- 生产运行提交以 `<DEPLOY_DIR>/release-sha.txt` 记录；脚本只在新容器健康检查、JAR 校验和重启次数检查全部通过后更新该文件。
