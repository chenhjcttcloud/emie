...
| 项目 | 记录 |
|---|---|
| 状态 | 待发布；本地已修改，尚未提交、推送或部署 |
| 目标分支 | `project_manager_system` |
| 当前基线 | `d22e955785cc88608e40dc5a14b396a4b209de6a`，与远端业务分支一致 |
| 生产基线 | `d888b9b244dd8c03a1c6c51475a8311fac35d7df`（本轮未核验、未修改） |
| 本地验证 | `git diff --check` 通过；`./mvnw clean package` 成功，48 项测试全部通过；本地服务自动重载后 HTTP 200 |
| 数据库迁移 | 待发布时执行 `db/migrations/2026-07-15-add-scoring-review-workflow.sql` |
| 飞书结构变更 | 待发布时为项目、子任务、评分记录的主表与备份表补齐两级审核字段 |
...
- 本条仅记录待发布范围；生产备份、迁移、飞书字段创建、全量重刷和业务验证均尚未执行。
