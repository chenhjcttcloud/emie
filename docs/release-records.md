...
## 2026-09-24 月度绩效 Excel 下载修复（发布成功，1.14.0）

- 根因：绩效 Excel 按钮使用原生下载链接访问需 `X-Auth-Token` 的受保护接口，未带登录令牌；生产和测试均会得到鉴权失败响应。服务端异常也被错误包装成月份格式错误。
- 修复：下载改为带令牌的 fetch + Blob 保存；服务端仅对非法月份返回 400，生成异常返回 500；增加前端鉴权契约回归测试。
- 本地验证：定向 15 项测试通过，`./scripts/test-update.sh` 的测试容器、MySQL、Redis、应用健康检查、浏览器入口与 6 项弹窗回归通过；生产发布构建全量 355 项测试通过（0 失败、1 项跳过）。
- 提交 SHA：`19922202eca2b6aa11e9f52de65c767334d8fff5`，Gitee `master`、GitHub `main` 已同步。
- 生产结果：旧 SHA `38c196c2b67269d8c4481326551cf2222606b007` 原子切换成功，版本 `1.14.0`；备份 `/home/emie/emie-deploy-backups/20260924_165428`（732428 字节），旧容器 `emie-app-old-20260924_165428` 保留；JAR SHA-256 `7d7326a82a7eef0f82f54350a525e48b11e82b25a5a0911c36902d527d12274d`，公网 HTTP 200，发布后预检运行 SHA 一致。

## 2026-09-24 子任务积分与验收流程调整（发布成功，1.13.9）

- 计划变更：设计师提交子任务成果时自动入账已选择的基础积分并保持幂等；公司常规品由产品企划验收，渠道定制在产品企划后由销售验收；管理员不参与正常子任务验收。设计/送审需求创建时选择积分规则，设计师交付时入账，并由需求创建人和对应产品企划分别验收或驳回，不包含评分。
- 同步清理废弃的评分、难度统计与相关界面入口；保留工作量统计的明细查看能力。
- 数据库迁移：计划执行 V80，新增设计需求积分快照和双验收状态；正式发布后补充生产备份位置、迁移验证、提交 SHA 与容器/公网验证结果。
- 提交 SHA：`38c196c2b67269d8c4481326551cf2222606b007`，前序提交 `a98c9738`（设计师月度绩效 Excel）；Gitee `master` 与 GitHub `main` 均已同步。
- 本地验证：Java 21 全量构建 355 项测试通过（0 失败、1 项跳过）；`./scripts/test-update.sh` 的测试容器、MySQL、Redis、静态资源、浏览器入口及 6 项弹窗回归通过。
- 数据库迁移：V80 成功；发布前数据库备份 `/home/emie/emie-deploy-backups/20260924_150131`，727576 字节。
- 生产结果：旧 SHA `9f66cc335d773f392f54963d68d7b1b8b0f86ae8` 原子切换到 `38c196c2b67269d8c4481326551cf2222606b007`；旧容器 `emie-app-old-20260924_150131` 保留。版本 `1.13.9`，JAR SHA-256 `37cf33a0a89aab97f8fa971b40ac8383d05a297a8db98167ef01dd8f251cbf64`。
- 发布验证：数据库备份 727576 字节；切换 62 秒、远端总耗时 71 秒；生产脚本健康检查成功，公网 HTTP 200，发布后预检读取到生产运行 SHA 与目标一致。

本文件按时间倒序记录代码推送和生产部署。记录中不得填写服务器密码、数据库密码、令牌、私钥或完整环境变量内容。

## 2026-09-03 取消图片上传前压缩（发布成功，1.10.6）

- 变更内容：公共上传入口取消 JPEG/WebP 的 Canvas 缩放和质量重编码，所有上传场景直接提交用户选择的原文件；列表和详情继续使用独立缩略图。
- 提交 SHA：c5508e4c3674e656c72413c68bbeb7f5e5291261；Gitee master、GitHub main 已同步。
- 本地验证：Java 21 完整构建与 276 项测试通过；测试容器、MySQL、Redis、静态资源、浏览器入口和 6 项弹窗回归通过。
- 数据库迁移：无；发布前数据库备份位于 /home/emie/emie-deploy-backups/20260903_182106，备份大小 468169 字节。
- 生产结果：旧 SHA 93e416c8d87f9abdfa0c5dcb476e97780bd8f542 原子切换至目标 SHA，生产版本 1.10.6，产物 SHA-256 为 2d02b02013d00296dd0b4803e468f73fba62a4f100f1075b7a7583f998b7953f。
- 发布验证：原子切换 76 秒、远程总耗时 83 秒；公网 HTTP 200；应用与预览容器均为 running、重启数 0、OOM false；线上 bootstrap.js?v=1130、core-runtime.js?v=284，已确认不含上传压缩函数。
- 已知边界：此前已被压缩上传的历史图片无法从服务器自动恢复，需用户用本地原文件重新上传。

## 2026-09-02 飞书 V2 单向镜像重构（发布成功，1.9.5）

- 变更内容：新增独立 V2 八表契约、真实附件上传与 token 缓存、主表全量对账、备份表业务增量、删除留档，以及 staging 预创建、原子激活和回滚接口。
- 提交 SHA：`7bfc2182409502053ad7c4de016b2f2eb1dadf91`；Gitee `master`、GitHub `main` 已同步。
- 数据库迁移：Flyway V74 成功创建 `feishu_attachment_cache`，生产 schema 当前版本 74。
- 生产结果：旧 SHA `252247438c82c4be299ceee0dcf31479220910aa` 原子切换成功，生产版本 `1.9.5`，公网 HTTP 200。
- 备份目录：`/home/emie/emie-deploy-backups/20260902_142504`，数据库备份 421300 字节；旧容器 `emie-app-old-20260902_142504`。
- 验证：Java 21 构建与 274 项测试通过；产物 SHA-256 `be9ccd9f7d4f868ba0bf5cd862bde7aea178e56980446f66fb0e6f646be381d3`；原子切换 51 秒、远程总耗时 59 秒。
- 飞书切换状态：应用已发布但 V2 staging Base 尚未激活，旧同步配置继续生效，不影响业务。

## 2026-09-02 图片下载与复制修复（发布成功，1.9.4）

- 变更内容：恢复图片原生认证下载，保留原始文件内容和上传文件名；预览、复制和拖拽载荷增加图片 MIME 校验。
- 本地验证：`./scripts/test-update.sh` 通过，应用、MySQL、Redis、静态资源、浏览器入口及 6 项弹窗回归正常。
- 数据库迁移：无。
- 提交 SHA：`252247438c82c4be299ceee0dcf31479220910aa`；Gitee `master`、GitHub `main` 已同步。
- 生产结果：旧 SHA `3da65e22fb24358c962037f8119a12d6b3334281` 原子切换成功，生产版本 `1.9.4`，公网 HTTP 200。
- 备份目录：`/home/emie/emie-deploy-backups/20260902_114756`，数据库备份 416901 字节；旧容器 `emie-app-old-20260902_114756`。
- 发布验证：Java 21 构建与 272 项测试通过；产物 SHA-256 为 `4a16ab393377f5172f29133f1b60c376895e1f7e4ee889df8cf11a1208fd85e9`；原子切换 45 秒、远程总耗时 53 秒；应用和预览容器均为 running、重启数 0、OOM false，静态资源版本核验通过。

## 2026-09-02 飞书同步 V2 兼容层（已发布）

- 变更内容：集中字段契约、只读结构预检、非关键字段兼容列、操作日志统一转换及单条失败任务重试。
- 提交 SHA：`3da65e22fb24358c962037f8119a12d6b3334281`；Gitee `master`、GitHub `main` 已同步。
- 生产结果：旧 SHA `f9ab54bd0338a60f3b8e4fbae6fd1773a12c689d` 原子切换成功，生产公网 HTTP 200，版本 `1.9.3`。
- 数据库备份：`/home/emie/emie-deploy-backups/20260902_102911`，备份大小 413158 bytes；无数据库迁移。
- 验证：Java 21 完整构建、272 项测试、本地容器和浏览器回归均通过；生产 `sub_task #1081` 重试后队列状态为 `done`、重试次数 0。

## 2026-09-01 飞书删除时间字段类型校验修复（已发布）

- 修复内容：将“源数据删除时间”纳入飞书日期字段类型推断，避免子任务备份同步时报字段类型不匹配。
- 提交 SHA：`f9ab54bd0338a60f3b8e4fbae6fd1773a12c689d`。
- 推送结果：Gitee `master`、GitHub `main` 均已同步该提交。
- 生产结果：旧 SHA `4ccb46859b0df728901f7d6601fb53fc78cf98b5` 已原子切换至目标 SHA；生产公网 HTTP 200。
- 数据库备份：`/home/emie/emie-deploy-backups/20260901_173146`，备份大小 408247 bytes；无数据库迁移。
- 验证：Java 21 构建成功，268 项测试通过，生产发布脚本返回 `release_status=success`。
...
## 2026-08-31 移除旧任务验收积分规则（发布成功，1.7.8）

| 项目 | 记录 |
|---|---|
| 状态 | 发布成功 |
| 业务分支 | 本地 `master`；Gitee `master`；GitHub `main` |
| 变更范围 | 删除旧 `TASK_APPROVED` 通用验收积分规则，取消未选择积分规则时自动回退 10 分的逻辑；子任务积分仅按创建时选择的具体规则计算。 |
| 本地验证 | Java 21 完整构建成功，255 项测试通过；`./scripts/test-update.sh` 通过；应用、MySQL、Redis、静态资源和浏览器检查通过。 |
| 数据库迁移 | `V72__remove_legacy_task_approved_point_rule.sql`。 |
| 旧提交 SHA | `3618bb4a5048912113c1a8997b3c69fc7b2cacd9` |
| 发布提交 SHA | `0bfe540d23c4e526f82fae48a7d520ccfe13a82f` |
| 生产运行 SHA | `0bfe540d23c4e526f82fae48a7d520ccfe13a82f` |
| 备份目录 | `/home/emie/emie-deploy-backups/20260831_142130`，数据库备份 441000 字节；旧容器 `emie-app-old-20260831_142130`。 |
| 发布验证 | 原子切换 44 秒，远程总耗时 50 秒；公网 HTTP 200；生产版本从 `1.7.7` 更新为 `1.7.8`；生产 JAR SHA-256 为 `beef48010b3c488cb6a0440efde95e87a55f6fbb28b386024c202d4c05bb7065`。 |

## 2026-08-31 图档库与素材市场增强（发布成功，1.7.7）

| 项目 | 记录 |
|---|---|
| 状态 | 发布成功 |
| 业务分支 | 本地 `master`；Gitee `master`；GitHub `main` |
| 变更范围 | 新增图档库、图片与 AI 文件上传预览、单个及批量下载、产品企划/管理员维护权限，以及设计师交付成果关联图档库；同步完善素材市场采纳、点赞、分类和积分规则。 |
| 本地验证 | `./scripts/test-update.sh` 通过，应用、MySQL、Redis、静态资源和浏览器冒烟检查通过；生产 Java 21 完整构建成功，255 项测试通过。 |
| 数据库迁移 | `V62__material_market_adoption_and_likes.sql` 至 `V71__create_image_library_items.sql`。 |
| 旧提交 SHA | `8673ca11b04ad7ba2fdad5e1dc68676eb7e9d065` |
| 发布提交 SHA | `3618bb4a5048912113c1a8997b3c69fc7b2cacd9` |
| 生产运行 SHA | `3618bb4a5048912113c1a8997b3c69fc7b2cacd9` |
| 备份目录 | `/home/emie/emie-deploy-backups/20260831_140749`，数据库备份 439724 字节；旧容器 `emie-app-old-20260831_140749`。 |
| 发布验证 | 原子切换 52 秒，远程总耗时 61 秒；公网 HTTP 200；生产版本从 `1.7.6` 更新为 `1.7.7`；生产 JAR SHA-256 为 `1763565312bc0ddc4074744a18a1a0e6c2b97e9a806a0c46506b68ab74e9c9f4`。 |

## 2026-08-28 第二阶段服务拆分与基础治理（发布成功，1.7.6）

| 项目 | 记录 |
|---|---|
| 状态 | 发布成功 |
| 业务分支 | 本地 `master`；Gitee `master`；GitHub `main` |
| 变更范围 | 子任务与项目生命周期命令逻辑从 `ProjectService` 迁入独立服务；统一前端 API 错误、弹窗、身份上下文和刷新入口；收紧飞书同步管理权限与令牌传输；更新数据库结构校验和发布文档。 |
| 本地验证 | Java 21 完整构建成功，248 项测试通过；`./scripts/test-update.sh` 通过；应用、MySQL、Redis、静态资源、浏览器入口和 6 项弹窗回归通过。 |
| 数据库迁移 | 无 |
| 旧提交 SHA | `848c1d40f8db8738b234d8025cf35813c3eb6daa` |
| 发布提交 SHA | `8673ca11b04ad7ba2fdad5e1dc68676eb7e9d065` |
| 生产运行 SHA | `8673ca11b04ad7ba2fdad5e1dc68676eb7e9d065` |
| 备份目录 | `/home/emie/emie-deploy-backups/20260828_160431`，数据库备份 428395 字节；旧容器 `emie-app-old-20260828_160431`。 |
| 发布验证 | 原子切换 50 秒，远程总耗时 63 秒；公网 HTTP 200；生产版本从 `1.7.5` 更新为 `1.7.6`。 |

## 2026-08-14 静态资源缓存版本更新（发布成功，1.4.0）

| 项目 | 记录 |
|---|---|
| 状态 | 发布成功 |
| 业务分支 | `project_manager_system` |
| 变更范围 | 收敛并提交此前已验证的业务改动；静态资源缓存版本从 `v601` 更新为 `v602`。 |
| 本地验证 | Java 21 全量 162 项测试通过；`./scripts/test-update.sh` 通过；应用、MySQL、Redis、健康检查、浏览器入口和 6 项弹窗回归均通过。 |
| 数据库迁移 | 无 |
| 旧提交 SHA | `dc676ed10de68dbe17c08652031d30b378199cbc` |
| 新提交 SHA | `bd3c740f3d826ca1070418303bcbadaa70d299d5` |
| 生产运行 SHA | `bd3c740f3d826ca1070418303bcbadaa70d299d5` |
| 备份目录 | `/home/emie/emie-deploy-backups/20260814_190930`，数据库备份 322312 字节；旧容器 `emie-app-old-20260814_190930`。 |
| 发布验证 | 原子切换耗时 39 秒，远程总耗时 45 秒；公网 HTTP 200；生产版本从 `1.3.9` 更新为 `1.4.0`。 |

## 2026-08-13 文件下载与管理看板修复（发布成功，1.3.3）

| 项目 | 记录 |
|---|---|
| 状态 | 发布成功 |
| 业务分支 | `project_manager_system` |
| 变更范围 | 修复设计图下载鉴权与原文件名；收紧 admin 文件匿名访问；优化工作量表格滚动；修复日志快速翻页请求乱序；修复设计师配置名称匹配。 |
| 本地验证 | `./scripts/test-update.sh` 通过；Java 21 全量 158 项测试通过；浏览器入口和弹窗回归通过。 |
| 数据库迁移 | 无 |
| 旧提交 SHA | `23c7b8fb81fa0abaa50565db85685fa6a9a8c77c` |
| 新提交 SHA | `d1fb4cac37e8bf699c3dcdf2d1c3ab1737987fc7` |
| 生产运行 SHA | `d1fb4cac37e8bf699c3dcdf2d1c3ab1737987fc7` |
| 备份目录 | `/home/emie/emie-deploy-backups/20260813_115149`，数据库备份 `302239` 字节；旧容器 `emie-app-old-20260813_115149`。 |
| 发布验证 | 切换耗时 43 秒，远程总耗时 50 秒；公网 HTTP 200；生产版本 `1.3.3`。 |

## 2026-08-13 积分月份自动归属与任务表单优化（发布成功，1.3.4）

| 项目 | 记录 |
|---|---|
| 状态 | 发布成功 |
| 业务分支 | `project_manager_system` |
| 变更范围 | 积分按子任务实际完成月份自动归属；积分规则和难度档位暂不启用时可留空；移除任务表单中的手动积分归属月份。 |
| 本地验证 | Java 21 全量 158 项测试通过；本地容器、静态资源、浏览器入口和弹窗回归通过。 |
| 数据库迁移 | 无 |
| 旧提交 SHA | `d1fb4cac37e8bf699c3dcdf2d1c3ab1737987fc7` |
| 新提交 SHA | `e7e877e880d830fe76fd700d01ba6eb894fc0660` |
| 生产运行 SHA | `e7e877e880d830fe76fd700d01ba6eb894fc0660` |
| 备份目录 | `/home/emie/emie-deploy-backups/20260813_141914`，数据库备份 `303017` 字节；旧容器 `emie-app-old-20260813_141914`。 |
| 发布验证 | 切换耗时 41 秒，远程总耗时 47 秒；公网 HTTP 200；生产版本 `1.3.4`。 |

## 2026-08-13 移除普通子任务中的自主提案选项（发布成功，1.3.5）

| 项目 | 记录 |
|---|---|
| 状态 | 发布成功 |
| 业务分支 | `project_manager_system` |
| 变更范围 | 移除普通项目添加子任务表单中遗留的“自主提案任务”复选项；自主提案继续使用独立流程。 |
| 本地验证 | Java 21 全量 158 项测试通过；本地容器、静态资源、浏览器入口和弹窗回归通过。 |
| 数据库迁移 | 无 |
| 旧提交 SHA | `e7e877e880d830fe76fd700d01ba6eb894fc0660` |
| 新提交 SHA | `827ef2cfe02581efcc901e06619d6ceb289447d1` |
| 生产运行 SHA | `827ef2cfe02581efcc901e06619d6ceb289447d1` |
| 备份目录 | `/home/emie/emie-deploy-backups/20260813_142432`，数据库备份 `303044` 字节；旧容器 `emie-app-old-20260813_142432`。 |
| 发布验证 | 切换耗时 40 秒，远程总耗时 46 秒；公网 HTTP 200；生产版本 `1.3.5`。 |

## 2026-08-12 工作台统计、日志与工作量修复（发布成功，1.3.2）

| 项目 | 记录 |
|---|---|
| 状态 | 发布成功 |
| 业务分支 | `project_manager_system` |
| 变更范围 | 统一工作台统计卡片与评分中心口径；修复产品企划视角切换及卡片跳转筛选；补齐评分分页；修复系统管理日志渲染；校验工作量统计并限制管理员访问；删除未使用的旧待评分查询。 |
| 本地验证 | Java 21 构建成功，158 项测试通过；本地容器、数据库/Redis、静态资源、浏览器冒烟和弹窗回归通过。 |
| 数据库迁移 | 无 |
| 旧提交 SHA | `7df331b32fb0f929f54a8b86718d9460ca4e0326` |
| 新提交 SHA | `23c7b8fb81fa0abaa50565db85685fa6a9a8c77c` |
| 生产运行 SHA | `23c7b8fb81fa0abaa50565db85685fa6a9a8c77c` |
| 备份目录 | `/home/emie/emie-deploy-backups/20260812_161956`，数据库备份 `295174` 字节；旧容器 `emie-app-old-20260812_161956`。 |
| 发布验证 | 切换耗时 41 秒，远程总耗时 46 秒；公网 HTTP 200；生产版本 `1.3.2`。 |

## 2026-08-12 设计需求项目群补全（发布成功，1.3.1）

| 项目 | 记录 |
|---|---|
| 状态 | 发布成功 |
| 业务分支 | `project_manager_system` |
| 变更范围 | 设计需求详情增加创建、进入和解散项目群；补充 `design_requirements` 群聊字段、Flyway V34 迁移、管理员/对应产品企划权限和飞书群聊接口。 |
| 本地验证 | `./scripts/test-update.sh` 通过；158 项测试通过；应用、MySQL、Redis、静态资源和浏览器回归均通过。 |
| 数据库迁移 | `src/main/resources/db/migration/V34__design_requirement_project_chat.sql`；生产切换前自动备份，备份目录见发布脚本输出。 |
| 旧提交 SHA | `672aff340d8989777e934d897db595ac18bfbcc1` |
| 新提交 SHA | `7df331b32fb0f929f54a8b86718d9460ca4e0326` |
| 生产运行 SHA | `7df331b32fb0f929f54a8b86718d9460ca4e0326` |
| 备份目录 | `/home/emie/emie-deploy-backups/20260812_134736`，数据库备份 `289666` 字节；旧容器 `emie-app-old-20260812_134736`。 |
| 发布验证 | 切换耗时 42 秒，远程总耗时 48 秒；公网 HTTP 200；生产版本 `1.3.1`；发布脚本报告 `生产发布完成：7df331b`。 |

## 2026-08-12 1.3.0 设计流程与积分绩效综合更新（发布成功）

| 项目 | 记录 |
|---|---|
| 状态 | 发布成功 |
| 业务分支 | `project_manager_system` |
| 变更范围 | 设计需求交付/终止/附件、积分绩效配置、通知链路、角色权限和生产数据兼容修复；补充自主开发提案图片字段及相关页面逻辑。 |
| 本地验证 | 全量 158 项测试通过；本地容器、数据库/Redis、静态资源和浏览器回归通过。 |
| 数据库迁移 | `V33__add_proposal_design_images.sql`；同时修正 `point_ledgers.points` 与生产 `DECIMAL(12,2)` 类型校验不一致问题。 |
| 旧提交 SHA | `613557bd101f8a1b3fddc82a725267f59d5ad9fc` |
| 新提交 SHA | `672aff340d8989777e934d897db595ac18bfbcc1` |
| 生产运行 SHA | `672aff340d8989777e934d897db595ac18bfbcc1` |
| 发布过程 | 首次候选因生产历史迁移量大启动超时，自动回滚；修复积分流水字段映射后重新发布成功。 |
| 发布验证 | 数据库备份和原子切换通过；公网 HTTP 200；生产版本从 `1.2.9` 更新为 `1.3.0`。 |

## 2026-08-07 飞书同步待处理队列修复（发布成功）

| 项目 | 记录 |
|---|---|
| 状态 | 发布成功 |
| 业务分支 | `project_manager_system` |
| 变更范围 | 修复新同步任务的 `nextRetryAt` 为空时无法被 Worker 查询和领取的问题；保留失败任务原有退避重试逻辑。 |
| 本地验证 | Java 21 `clean package` 成功，121 项测试通过；`git diff --check` 通过。 |
| 数据库迁移 | 无 |
| 旧提交 SHA | `b98e0a86785a5029f9dea93870055f241657818a` |
| 新提交 SHA | `27818b0ed699175a96710d3b16419907b37fb900` |
| 远端核对 SHA | `27818b0ed699175a96710d3b16419907b37fb900` |
| 生产运行 SHA | `27818b0ed699175a96710d3b16419907b37fb900` |
| 数据库备份 | `/home/emie/emie-deploy-backups/20260807_171158`，236583 字节且压缩校验通过。 |
| 旧容器回滚点 | `emie-app-old-20260807_171158` |
| 生产验证 | `emie-app` running、restart=0、OOM=false；健康接口和公网均 HTTP 200；版本 `1.2.9`；Worker 启动后成功领取 20 条待处理任务。 |
| 备注 | 管理概览中的 `processing` 可能因同一事务内快速流转而观察不到，不影响任务实际领取和同步。 |

## 2026-08-06 缩略图连接池与飞书同步修复（发布成功）

| 项目 | 记录 |
|---|---|
| 状态 | 发布成功 |
| 业务分支 | `project_manager_system` |
| 变更范围 | 详情页缩略图懒加载、文件权限轻量查询、飞书同步事务边界、版本检查 SSE 降级为轮询 |
| 本地验证 | `git diff --check`、`./mvnw test`，121 项通过 |
| 数据库迁移 | 无 |
| 旧提交 SHA | `2db369fd7e47f075c2c002818fa1ecd88df17c5c` |
| 新提交 SHA | `21886ba53775b7a05cf74225939090337f15765b` |
| 远端核对 SHA | `21886ba53775b7a05cf74225939090337f15765b` |
| 生产运行 SHA | `21886ba53775b7a05cf74225939090337f15765b` |
| 数据库备份 | `/home/emie/emie-deploy-backups/20260806_135611`，229503 字节 |
| 旧容器回滚点 | `emie-app-old-20260806_135611` |
| 发布耗时 | 远程 72 秒，切换 66 秒 |
| 生产验证 | `emie-app` running；健康接口 HTTP 200；公开配置版本 `1.1.9`；入口 `bootstrap.js?v=337`；发布后 3 分钟无新增严重错误 |
| 备注 | `emie-preview-converter` 在发布前即不在当前 Docker 容器列表，本次应用发布未修改该服务 |

## 2026-08-06 无损缩略图与响应式显示（发布成功）

| 项目 | 记录 |
|---|---|
| 状态 | 发布成功 |
| 变更范围 | PNG 无损缩略图、640px 最大边、响应式缩略图尺寸、缓存格式切换 |
| 本地验证 | `git diff --check`、前端 JS 语法检查、`./mvnw test`，121 项通过 |
| 数据库迁移 | 无 |
| 新提交 SHA | `916c90dcc7487332a914990bd8058025ba964811` |
| 远端核对 SHA | `916c90dcc7487332a914990bd8058025ba964811` |
| 生产运行 SHA | `916c90dcc7487332a914990bd8058025ba964811` |
| 数据库备份 | `/home/emie/emie-deploy-backups/20260806_140510`，229543 字节 |
| 旧容器回滚点 | `emie-app-old-20260806_140510` |
| 生产验证 | `emie-app` running，restart=0，OOM=false；健康接口 HTTP 200；版本 `1.2.0`；发布后 3 分钟无严重错误 |

## 2026-08-06 低分辨率桌面响应式补强（发布成功）

| 项目 | 记录 |
|---|---|
| 状态 | 发布成功 |
| 变更范围 | 1280/1100/900px 桌面断点、4:3 屏幕布局、表格和弹窗防溢出 |
| 本地验证 | `git diff --check`、前端 JS 语法检查、`./mvnw test`，121 项通过 |
| 数据库迁移 | 无 |
| 新提交 SHA | `55be0cbeb228a853a5092065a857dafa53c9de2a` |
| 远端核对 SHA | `55be0cbeb228a853a5092065a857dafa53c9de2a` |
| 生产运行 SHA | `55be0cbeb228a853a5092065a857dafa53c9de2a` |
| 数据库备份 | `/home/emie/emie-deploy-backups/20260806_140824`，229556 字节 |
| 旧容器回滚点 | `emie-app-old-20260806_140824` |
| 生产验证 | 版本 `1.2.1`；健康接口 HTTP 200；容器 running、restart=0、OOM=false；发布后 3 分钟无严重错误 |


---

## 2026-07-30 标准快速生产发布脚本

| 项目 | 记录 |
|---|---|
| 状态 | 检查完成 |
| 变更范围 | 新增本地一键发布入口和服务器原子切换执行器；使用增量上传、稳定运行时镜像、版本化只读 JAR、候选容器预创建、1 秒健康轮询和状态机回滚，替代临时命令与逐版镜像复制。 |
| 安全边界 | 强制生产环境标识、正确分支、干净工作区、本地/远端 SHA 一致、当前生产健康、完整运行配置、JAR 校验、数据库备份、候选挂载及新容器重启次数检查；失败不更新 `release-sha.txt`。 |
| 本地验证 | 两个脚本 `bash -n` 通过；新增 `ProductionReleaseScriptTest` 覆盖候选先于停机、禁止逐版镜像构建、版本化只读 JAR、重复提交快速退出、目标 JAR 路径约束、空环境过滤及回滚后退出；Java 21 `clean package` 成功，117 项测试全部通过。 |
| 生产预检 | `--preflight-only` 已连接当前本地生产服务器，确认运行提交 `55e222d5b6b7071b10f4f0ada382093967ed0705`、SSH、sudo、本机健康和外网 HTTPS HTTP 200；预检没有构建、上传、停止或重启任何生产服务。 |
| 发布状态 | 脚本当前仅本地修改，尚未提交、推送或安装到生产；生产应用仍正常运行 `emie-app:55e222d`。 |

## 2026-07-30 设计/送审需求图片访问权限修复

| 项目 | 记录 |
|---|---|
| 状态 | 发布成功 |
| 操作时间 | 2026-07-30 18:21–18:51 CST |
| 旧生产提交 / 生产提交 | `78416c3bcb6a039bce361288b127c8ae2c6a5b6a` / `55e222d5b6b7071b10f4f0ada382093967ed0705` |
| Gitee 推送 | `project_manager_system` 已推送，功能提交的本地与远端完整 SHA 一致。 |
| 变更范围 | 将独立设计/送审需求纳入统一文件访问鉴权，需求负责人、产品企划和指定设计师可读取该需求的参考图、附件、交付参考图及交付附件；非参与人员仍拒绝访问。修复 `DR202607242846`（翻转相机）详情中“蓝V联动海报.png”已上传但非管理员角色无法显示的问题。 |
| 本地验证 | Java 21 `clean package` 成功，111 项测试全部通过；新增产品企划读取设计师交付图片的权限回归用例。 |
| 数据库 | 无结构或数据迁移；发布前备份 `/home/emie/emie-deploy-backups/20260730_183640/designpm.sql.gz`，217458 字节且 `gzip -t` 通过。 |
| 应用产物 | 本地、上传产物、镜像与生产容器 JAR SHA-256 均为 `eb5923e89da5c6391fe3e38d5653513edbfd8b8b968dd9a96586bd6a097dcc1c`。 |
| 回滚 | 旧 JAR 位于 `/home/emie/emie-deploy-backups/20260730_183640/app.jar`，旧镜像标签为 `emie-app:backup-20260730_183640`，最终切换保留的旧容器为 `emie-app-old-20260730_184549`。首次切换因运行环境数组包含空项而被 Docker 拒绝，旧版已恢复 HTTP 200；同时修正回滚后未立即退出的脚本缺陷并纠正运行提交标记，再次发布成功，数据库和上传文件未受影响。 |
| 生产验证 | `release-sha.txt` 与生产提交一致；`emie-app:55e222d` 持续 running、重启次数 0；容器 JAR 校验一致；本机公开配置及 `https://emie.emie.cn:9001/` 均 HTTP 200；启动后应用错误 0。目标需求仍关联 1 张交付参考图，目标 PNG 引用存在、原文件 1430008 字节；未登录缩略图和原图按预期返回 401。 |
| 验收边界 | 当前没有有效的需求参与者登录凭据或可接管的已登录浏览器会话，因此未代替真实用户完成目标图片的登录态点击验收；权限路径已有自动化回归、生产应用启动时 JPQL 解析、目标数据与文件交叉验证。 |

## 2026-07-30 设计/送审需求资料显示修复

| 项目 | 记录 |
|---|---|
| 状态 | 发布成功 |
| 操作时间 | 2026-07-30 17:53–18:11 CST |
| 旧生产提交 / 生产提交 | `1c2143db1c5f616aad14e6577cbbf65c06c92245` / `78416c3bcb6a039bce361288b127c8ae2c6a5b6a` |
| Gitee 推送 | `project_manager_system` 已推送，功能提交的本地与远端完整 SHA 一致 |
| 变更范围 | 设计/送审需求详情新增“需求资料”区域，并在“交付与评分”中展示设计师交付参考图和附件；复用项目/子任务统一的图片放大、原图复制、预览与下载能力，不按创建角色分叉。 |
| 本地验证 | 两次 Java 21 `clean package` 均成功，110 项测试全部通过；全部前端 JavaScript 语法、差异和缓存版本回归检查通过。 |
| 数据库 | 无结构或数据迁移；发布前备份 `/home/emie/emie-deploy-backups/20260730_175700/designpm.sql.gz`，229800 字节且 `gzip -t` 通过。用户最终确认反馈目标为需求 `DR202607242846`（翻转相机）；发布后仍关联 3 张需求参考图及 1 张设计师交付参考图，4 个生产原文件均存在且格式有效。 |
| 应用产物 | 本地 JAR、上传产物、镜像与生产容器 JAR SHA-256 均为 `cc7ed409dc3d3251435c2a0c478f0f6e73935116eda2f494a6bca13176f68469`。 |
| 回滚 | 旧 JAR 位于 `/home/emie/emie-deploy-backups/20260730_175700/app.jar`，旧镜像标签为 `emie-app:backup-20260730_175700`，停止的旧容器为 `emie-app-old-20260730_175700`。首次切换因运行配置提取规则漏掉数据库变量而未通过健康检查，已立即恢复旧容器并验证 HTTP 200；修正为逐项提取并校验 12 个业务变量后重新发布成功，数据库未受影响。 |
| 生产验证 | `release-sha.txt` 与功能提交一致；`emie-app:78416c3` 持续 running、重启次数 0；首页与公开配置 HTTP 200，主域名 `emie.emie.cn:9001` HTTPS 与证书校验通过；生产加载 `bootstrap.js?v=237`、`dashboard-lists.js?v=187`，详情展示函数已生效；未登录设计需求详情返回预期 401，MySQL、Redis、OpenResty正常，最终启动日志无应用严重错误。 |
| 验收边界 | 当前没有可接管的已登录浏览器会话，因此未代替真实用户完成登录态点击验收；生产数据关联、静态资源、接口、容器和日志已交叉验证。另只读发现 `product.emie.cn` 当前仍为非受信任证书，未纳入本次应用发布修改。 |

## 2026-07-30 交付成果弹窗缓存兼容热修
...
|---|---|
| 状态 | 发布成功 |
| 生产目标 / 生产提交 | 本地生产服务器 `<PRODUCTION_SERVER>` / `4b150f50c053ff694bf8883e139034c493eb080a` |
...
|---|---|
| 状态 | 发布成功 |
| 生产目标 / 生产提交 | 本地生产服务器 `<PRODUCTION_SERVER>` / `ec44b025cb1994c02f5246d6c20e875cf4f343dc` |
| Gitee 推送 | `project_manager_system` 已推送，功能提交的本地与远端完整 SHA 一致 |
...
| 生产验证 | `release-sha.txt` 与目标提交一致；容器 `emie-app:ec44b02` running、重启次数 0；本机公开配置和外网 HTTPS 入口 HTTP 200；生产加载 `admin-shell.js?v=151`；飞书跳转地址为 `<PUBLIC_SERVICE_URL>`，数据库展示配置为 MySQL 8.4.10 与 `<DATABASE_ENDPOINT>`。 |
| 回滚 | 未执行。 |
...
|---|---|
| 状态 | 发布成功 |
| 生产目标 | 本地生产服务器 `<PRODUCTION_SERVER>` |
...
| 操作人 | Codex |
| 业务分支 | `project_manager_system` |
| 旧提交 SHA | `55be0cbeb228a853a5092065a857dafa53c9de2a` |
| 新提交 SHA | `1a038020e895685de25654065be71d726c64dc5d` |
| 远端核对 SHA | `1a038020e895685de25654065be71d726c64dc5d` |
| 生产运行 SHA | `1a038020e895685de25654065be71d726c64dc5d` |
| 开始时间 | 2026-08-06 14:32（Asia/Shanghai） |
| 完成时间 | 2026-08-06 14:34（Asia/Shanghai） |
| 影响范围 | 系统管理内部导航单列、纵向滚动、窄屏换行和静态资源缓存版本 |
| 停机情况 | 无，原子切换约 66 秒 |

变更内容：

- 系统管理内部导航全部强制单列，所有分组和菜单保持在左侧导航面板。
- 导航面板支持纵向滚动，禁止横向溢出；长菜单名称在列内换行。
- “日志、分享管理、工作量”继续归属左侧“运维与审计”分组。
...
- [x] 未包含密码、令牌和本地生成文件
- [x] `git diff --check` 通过
- [x] `./mvnw clean package` 通过，121 项测试通过
- [x] 远端分支状态已同步
- [x] 旧提交 SHA 已记录
- [x] 生产预检通过，公网检查 HTTP 200
...
|---|---|
| 是否涉及迁移 | 否 |
| 备份文件 | `/home/emie/emie-deploy-backups/20260806_143307` |
| 备份验证 | 生产发布脚本验证通过，备份大小 229121 bytes |
| 迁移脚本 | 无 |
| 迁移结果 | 不涉及 |
| 数据验证 | 不涉及 |
...
- [x] 推送后本地与远端 SHA 一致
- [x] 生产运行 SHA 与计划发布 SHA 一致
- [x] `emie-app` 候选切换和健康检查通过
- [x] `emie-preview-converter` 保持运行
- [x] 公网 HTTP 检查通过（200）
- [x] 生产发布脚本返回 `release_status=success`

异常、处理与回滚：

- 异常：无；SSH 执行期间有 locale 警告，不影响发布。
- 处理：无
- 回滚提交：未执行
- 数据库恢复：不需要
- 回滚后验证：不适用
...
- 代码已推送 Gitee，生产已成功运行提交 `1a038020e895685de25654065be71d726c64dc5d`。
### 2026-08-06 16:37 - 项目群状态刷新与终止项目按钮修复
...
| 操作人 | Codex |
| 业务分支 | `project_manager_system` |
| 旧提交 SHA | `aa6fafa342b1e75451679da18fae077fd74bba33` |
| 新提交 SHA | `052ef8884701b00935010d2013debd8af2e68069` |
| 远端核对 SHA | `052ef8884701b00935010d2013debd8af2e68069` |
| 生产运行 SHA | `052ef8884701b00935010d2013debd8af2e68069` |
| 开始时间 | 2026-08-06 16:37（Asia/Shanghai） |
| 完成时间 | 2026-08-06 16:39（Asia/Shanghai） |
| 影响范围 | 项目群创建/解散后的详情刷新、终止项目群按钮显示、前端缓存版本 |
| 停机情况 | 无，原子切换约 37 秒 |

变更内容：

- 创建或解散项目群成功后将项目 ID 传入刷新流程，立即重新读取项目详情。
- 项目已终止或待终止时不再显示“创建项目群”按钮。
...
- [x] 未包含密码、令牌和本地生成文件
- [x] `git diff --check` 通过
- [x] `./mvnw -q test` 通过，121 项测试通过
- [x] 全部前端 JavaScript 语法检查通过
- [x] 远端分支 SHA 已核对
- [x] 生产预检通过，公网检查 HTTP 200
...
|---|---|
| 是否涉及迁移 | 否 |
| 备份文件 | `/home/emie/emie-deploy-backups/20260806_163732` |
| 备份验证 | 生产发布脚本验证通过，备份大小 231816 bytes |
| 迁移脚本 | 无 |
| 迁移结果 | 不涉及 |
| 数据验证 | 不涉及 |
...
- 项目群状态刷新和终止项目按钮修复已成功部署生产。
### 2026-08-07 10:23 - 快速切换页面渲染竞态修复
...
| 操作人 | Codex |
| 业务分支 | `project_manager_system` |
| 旧提交 SHA | `052ef8884701b00935010d2013debd8af2e68069` |
| 新提交 SHA | `b98e0a86785a5029f9dea93870055f241657818a` |
| 远端核对 SHA | `b98e0a86785a5029f9dea93870055f241657818a` |
| 生产运行 SHA | `b98e0a86785a5029f9dea93870055f241657818a` |
| 开始时间 | 2026-08-07 10:22（Asia/Shanghai） |
| 完成时间 | 2026-08-07 10:24（Asia/Shanghai） |
| 影响范围 | 快速切换页面时的前端渲染顺序和项目列表请求提交 |
| 停机情况 | 无，原子切换约 38 秒 |

变更内容：

- 增加页面渲染序列校验，旧请求返回后不得覆盖最新页面。
- 项目列表请求增加渲染序列校验，避免左侧高亮页面与右侧内容不一致。
- 更新 `core-runtime.js`、`dashboard-projects.js`、`dashboard-lists.js` 和缓存版本。
...
|---|---|
| 是否涉及迁移 | 否 |
| 备份文件 | `/home/emie/emie-deploy-backups/20260807_102330` |
| 备份验证 | 生产发布脚本验证通过，备份大小 231813 bytes |
| 迁移脚本 | 无 |
| 迁移结果 | 不涉及 |
| 数据验证 | 不涉及 |
...
# 2026-08-31 仅记录积分、人工核算绩效

- 修复内容：关闭自动绩效计算展示，子任务完成后仅记录积分；后续由管理员根据积分人工核算绩效。保留旧接口字段以兼容现有调用。
- 发布前验证：Java 21 构建通过，255 项测试通过；本地容器更新与健康检查通过。
- 提交 SHA：`c4cdfdbf0a2c0c2f53431fbd80bb4a72fa3470f8`。
- 推送结果：Gitee `master` 与 GitHub `main` 均已核对为该 SHA。
- 生产结果：已原子发布，生产运行 SHA 核对一致，版本 `1.8.0`，公网 HTTP 200。
- 说明：发布切换期间出现一次 SSH 连接中断，生产服务随后正常启动；最终预检通过，未发生数据回滚。
...
- 提交 SHA：`5751556950a2b56a59538e3f8cda45522d52cd29`。
- 推送结果：Gitee 和 GitHub 的 `project_manager_system` 均已核对为该 SHA。
- 生产结果：已原子发布，生产运行 SHA 核对一致，版本 `1.6.8`，公网 HTTP 200；生产发布脚本预检通过。
- 说明：生产发布脚本已完成数据库备份、构建、产物校验和候选容器切换；执行期间仅出现远端 locale 警告，不影响发布。

# 2026-08-20 发布记录补充（文档同步）

- 本补充仅记录已完成的 `5751556950a2b56a59538e3f8cda45522d52cd29` 发布，不改变生产运行产物。
## 2026-09-01 子任务积分规则与难度必填（已发布）

- 添加子任务时积分规则和难度档位改为必填，前后端均增加空值校验。
- 发布前验证：定向 Maven 测试通过，`./scripts/test-update.sh` 通过。
- 提交 SHA：`de13dcc87cba1a1aa4130e345e5a9f3e9e9ead5f`。
- 推送结果：Gitee `master` 与 GitHub `main` 均已核对为该 SHA。
- 生产结果：已原子发布，生产运行 SHA 一致，公网 HTTP 200。
- 数据库备份：`/home/emie/emie-deploy-backups/20260901_092543`，备份大小 391760 bytes。
## 2026-09-01 工作台搜索与子任务发布校验优化（已发布）

- 子任务列表改为批量读取交付版本和积分流水，搜索防抖调整为 350ms，并避免旧请求覆盖新结果。
- 发布子任务时积分规则和难度档位均改为必填，前后端同步校验。
- 发布前验证：定向 Maven 测试通过，`./scripts/test-update.sh` 通过。
- 提交 SHA：`8c7f220cbfca9a7b982cbbf0a13f3c44227c2f5b`。
- 推送结果：Gitee `master` 与 GitHub `main` 均已核对为该 SHA。
- 生产结果：已原子发布，生产运行 SHA 一致，公网 HTTP 200。
- 数据库备份：`/home/emie/emie-deploy-backups/20260901_140854`，备份大小 396166 bytes。
# 发布记录说明
...
- 变更：销售、产品推广和产品企划新增设计/选审需求统计；设计师显示未完成需求，并在完成任务中单列完成的需求；供应链不显示该列。
- 发布前验证：本地测试环境更新、前端语法检查、完整 Maven 构建与 349 项测试通过，1 项按既有规则跳过。
- 生产结果：原子发布成功，版本 `1.12.7`，生产运行 SHA 与目标一致，公网 HTTP 200，容器运行中、重启次数 0、OOM=false。
- 数据库：无新增迁移；发布脚本已备份，目录 `/home/emie/emie-deploy-backups/20260920_150538`，`designpm.sql.gz` 为 662041 bytes。
- 异常：同步队列定时任务出现连接泄漏检测告警，栈指向既有 `SyncWorker`；本次工作量改动未涉及该路径，发布后应用和接口正常，未执行回滚。

## 2026-09-20 工作量完成率合并口径（已发布）

- 生产提交：`dee955971e23e8511580e9457620855fb9aa526d`；完成率和绿色进度条按“子任务 + 设计/选审需求”总工作量计算。
- 发布前验证：前端语法检查、本地测试环境更新和生产 Java 21 完整构建通过；349 项测试通过，1 项既有跳过。
- 生产结果：原子发布成功，版本 `1.12.8`，生产运行 SHA 一致，公网 HTTP 200，容器运行中、重启次数 0、OOM=false。
- 数据库：无迁移；备份目录 `/home/emie/emie-deploy-backups/20260920_152209`，数据库备份 662183 bytes。

## 2026-09-16 重新交付按钮修复（已发布）

- 修复内容：设计师确认修改后，工作台和项目详情恢复显示“重新交付”，并调用重新交付接口，普通首次交付流程保持不变。
- 发布前验证：前端语法检查、定向测试及 Java 21 完整构建通过；341 项测试通过，1 项按既有规则跳过。
- 旧提交：`beae0b27941c5ecdfac5c83aad3f4cd963362c4a`。
- 生产提交：`e2de04f7fe3aa7d0ce2e9cbe879043c308e4a721`，版本 `1.11.5`。
- 推送结果：Gitee `master` 与 GitHub `main` 均已核对包含生产提交。
- 生产结果：原子发布成功，公网 HTTP 200；容器运行中，重启次数 0，未发生 OOM。
- 数据库：无迁移；发布前备份 `/home/emie/emie-deploy-backups/20260916_110601`，压缩后 615164 bytes。

## 2026-09-16 图档库分页与用户 ID 标识修正（已发布）

- 修复内容：图档库分页改为居中的紧凑胶囊条；用户管理第一列改为真实系统主键，历史内部登录标识明确为“账号ID”，身份切换器增加账号前缀。
- 发布前验证：JavaScript 语法、定向测试、`git diff --check`、本地测试容器、HTTP/静态资源、浏览器弹窗回归及 Java 21 完整构建通过；343 项测试无失败，1 项按既有规则跳过。
- 旧提交：`c6dfffd3cf52286010e793269d85245e1cd64708`。
- 生产提交：`b5422874c50756b7de24b977fc84bdb132ac8566`，版本 `1.11.8`。
- 推送结果：Gitee `master` 与 GitHub `main` 均已核对为生产提交。
- 生产结果：原子发布成功，公网 HTTP 200；真实登录页面确认系统 ID、账号标签和图档库分页均生效。
- 数据库：无迁移；发布前备份 `/home/emie/emie-deploy-backups/20260916_165301`，压缩后 624450 bytes；旧容器 `emie-app-old-20260916_165301` 保留用于回滚。
