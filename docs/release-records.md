...
| 当前生产提交 | `4c70d1752dbc80b78f6729e8480c8bf628968528` |
| 本地验证 | `git diff --check` 通过；`./mvnw clean package` 成功，54 项测试全部通过；本地 `8080` HTTP 200 |
| 数据库结构变更 | 无 |
| 飞书结构要求 | 主表新增文本字段“同步来源”；备份表新增文本字段“同步来源”“备份状态”和日期字段“源数据删除时间” |
| 生产操作 | 未提交、未推送、未修改飞书字段、未触发生产同步、未部署 |
...
- 发布时必须先调用字段补齐接口，再执行一次全量同步写入来源标记；首次全量只建立标记，后续对账才会安全清理历史孤儿记录。
