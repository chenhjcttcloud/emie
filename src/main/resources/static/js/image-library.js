...
async function renderImageLibrary(main) {
  ...
  try {
    ...
    }).join('') : '<div class="image-library-empty"><div class="image-library-empty-icon">🖼️</div><h3>还没有图档</h3><p>上传产品图片，建立团队共享的视觉资产库</p></div>'; pagination.hidden = filteredItems.length <= PAGE_SIZE; pagination.innerHTML = `<span>显示 ${filteredItems.length ? page * PAGE_SIZE + 1 : 0}-${Math.min((page + 1) * PAGE_SIZE, filteredItems.length)} / ${filteredItems.length}</span><div><button class="btn btn-outline btn-sm" ${page <= 0 ? 'disabled' : ''} data-page="${page - 1}">上一页</button><span>${page + 1} / ${totalPages}</span><button class="btn btn-outline btn-sm" ${page >= totalPages - 1 ? 'disabled' : ''} data-page="${page + 1}">下一页</button></div>`; };
