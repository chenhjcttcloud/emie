...
async function renderImageLibrary(main) {
  ...
  try {
    ...
    const pagination = document.getElementById('imageLibraryPagination'); let filteredItems = items, page = 0;
    const renderItems = () => { const totalPages = Math.max(1, Math.ceil(filteredItems.length / PAGE_SIZE)); page = Math.min(page, totalPages - 1); const list = filteredItems.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE); grid.innerHTML = list.length ? list.map(item => {
    ...
    }).join('') : '<div class="image-library-empty"><div class="image-library-empty-icon">🖼️</div><h3>还没有图档</h3><p>上传产品图片，建立团队共享的视觉资产库</p></div>'; pagination.hidden = filteredItems.length <= PAGE_SIZE; pagination.innerHTML = `<span class="image-library-page-range">${filteredItems.length ? page * PAGE_SIZE + 1 : 0}–${Math.min((page + 1) * PAGE_SIZE, filteredItems.length)}，共 ${filteredItems.length} 项</span><div class="image-library-page-controls"><button type="button" ${page <= 0 ? 'disabled' : ''} data-page="${page - 1}">← 上一页</button><span>${page + 1} / ${totalPages}</span><button type="button" ${page >= totalPages - 1 ? 'disabled' : ''} data-page="${page + 1}">下一页 →</button></div>`; };
    EMIE.imageLibraryItems = items; renderItems(); document.getElementById('imageLibrarySearch').addEventListener('input', e => { const q = e.target.value.trim().toLowerCase(); filteredItems = items.filter(item => `${item.name} ${item.ipName} ${(item.subOptions || []).join(' ')} ${getUserName(item.ownerUserId)}`.toLowerCase().includes(q)); page = 0; renderItems(); }); pagination.addEventListener('click', e => { const button = e.target.closest('button[data-page]'); if (!button || button.disabled) return; page = Number(button.dataset.page); renderItems(); grid.scrollIntoView({ behavior: 'smooth', block: 'start' }); });
