...
function materialProjectLink(m, compact = false) {
  ...
  return `<a href="javascript:void(0)" class="material-project-link" data-emie-action="click:market-open-project" data-project-id="${Number(m.projectId)}" title="点击查看关联项目">${label}</a>`;
}
...
function renderMaterialCard(m) {
  ...
  return `<article class="material-card ${chosen ? 'material-card-selected' : ''}" tabindex="0" data-emie-action="click:market-detail" data-material-id="${m.id}" aria-label="查看素材 ${escHtml(m.title || '')}">
    <div class="material-cover">${preview ? `<img src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==" data-auth-src="${escHtml(preview)}" alt="${escHtml(m.title || '素材预览')}" loading="lazy">` : '<span aria-hidden="true">🎨</span>'}<span class="material-selected-badge ${chosen ? 'is-claimed' : 'is-unclaimed'}">${chosen ? '已被认领' : '未被认领'}</span></div>
...
async function unpublishMaterial(id) { if (!await EMIE.actions.showSystemConfirm('确认下架这件作品吗？下架后其他人将无法认领。')) return; try { await apiPost(`/materials/${id}/withdraw`); closeMaterialDetail(); await EMIE.actions.render(); } catch (e) { EMIE.actions.showSystemAlert('下架失败：' + e.message); } }
...
if (registerEventAction) {
  ...
  registerEventAction('market-select', (_event, el) => selectMaterial(Number(el.dataset.materialId)));
