...
function renderMaterialCard(m) {
  ...
  return `<article class="material-card ${chosen ? 'material-card-selected' : ''}" tabindex="0" data-emie-onclick="openMaterialDetail(${m.id})" aria-label="查看素材 ${escHtml(m.title || '')}">
    <div class="material-cover">${preview ? `<img src="${escHtml(preview)}" alt="${escHtml(m.title || '素材预览')}" loading="lazy">` : '<span aria-hidden="true">🎨</span>'}${chosen ? '<span class="material-selected-badge">已被选中</span>' : ''}</div>
    <div class="material-card-body"><h3>${escHtml(m.title || '未命名素材')}</h3><div class="material-meta">${escHtml(materialIp(m))}</div><p>${escHtml(m.description || '暂无产品说明')}</p><div class="material-card-foot"><span>作者：${escHtml(m.authorName || m.designerName || '-')}</span><span>${m.createdAt ? new Date(m.createdAt).toLocaleDateString('zh-CN') : ''}</span></div></div>
  </article>`;
}
...
async function openMaterialDetail(id) {
  ...
  overlay.innerHTML = `<div class="modal modal-lg"><div class="modal-header"><div class="modal-title">${escHtml(m.title || '素材详情')}</div><button class="modal-close" data-emie-onclick="closeMaterialDetail()">✕</button></div><div class="modal-body">
    ...
    <div class="material-detail-summary"><div><div class="detail-label">系统类 IP</div><div class="detail-value">${escHtml(materialIp(m))}</div></div><div><div class="detail-label">创意作者</div><div class="detail-value">${escHtml(m.authorName || m.designerName || '暂无')}</div></div></div>
...
function renderMaterialUploadModal() {
  ...
  overlay.innerHTML = `<div class="modal modal-lg"><div class="modal-header"><div class="modal-title">发布新素材</div><button class="modal-close" data-emie-onclick="closeMaterialUpload()">✕</button></div><form id="materialUploadForm" class="modal-body">
...
async function submitMaterialUpload() { const form = document.getElementById('materialUploadForm'); if (!form.reportValidity()) return; if (EMIE.projectState.uploadingCount > 0) return EMIE.actions.showSystemAlert('文件正在上传中，请等待上传完成'); if (!EMIE.projectState.materialRefImages.length) return EMIE.actions.showSystemAlert('请至少上传一张参考图片'); const btn = document.querySelector('#materialUploadModal .btn-primary'); btn.disabled = true; try { const fd = new FormData(form); await apiPost('/materials', { title: fd.get('title'), ipName: fd.get('ipName'), ipSubOptions: fd.get('ipSubOptions'), description: fd.get('description'), filesJson: JSON.stringify(EMIE.projectState.materialAttachments), referenceImagesJson: JSON.stringify(EMIE.projectState.materialRefImages) }); closeMaterialUpload(); await EMIE.actions.render(); } catch (e) { const error = document.getElementById('materialUploadError'); error.hidden = false; error.textContent = e.message; btn.disabled = false; } }
