...
async function openMaterialDetail(id) {
  ...
  overlay.innerHTML = `<div class="modal modal-lg"><div class="modal-header"><div class="modal-title">${escHtml(m.title || '素材详情')}</div><button class="modal-close" data-emie-action="click:market-close-detail">✕</button></div><div class="modal-body">
    ...
    <section class="material-detail-section"><h3>设计构思</h3><div class="material-detail-section-content"><div class="detail-value" style="margin-bottom:8px">AI 参与制图：${m.aiAssisted ? '是' : '否'}</div><div class="material-description">${m.description ? escHtml(m.description) : '<span class="material-empty">暂无</span>'}</div></div></section>
...
EMIE.registerModule('materialMarket', { renderMaterialMarket, openMaterialDetail, closeMaterialDetail, adoptMaterial, toggleMaterialLike, renderMaterialUploadModal, openMaterialEdit, deleteMaterial, unpublishMaterial, closeMaterialUpload, submitMaterialUpload, filterMaterials, setMaterialStatusFilter, setMaterialCategoryFilter, resetMaterialFilters, onMaterialIpChange, onMaterialAiToggle, toggleMaterialIpSubOption, handleMaterialRefImages, handleMaterialAttachments });
