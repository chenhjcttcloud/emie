package com.emie.designpm.designrequirement.repository;
...
public interface DesignRequirementRepository extends JpaRepository<DesignRequirement, Long> {
    /** 行锁读取（PESSIMISTIC_WRITE）：评分提交时串行化对同一需求的并发复评。 */
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT d FROM DesignRequirement d WHERE d.id = :id")
    Optional<DesignRequirement> findByIdForUpdate(@Param("id") Long id);
    ...
    Page<DesignRequirement> findPage(
    ...
    long countVisibleFileReferences(@Param("userId") String userId, @Param("storedName") String storedName);
}
