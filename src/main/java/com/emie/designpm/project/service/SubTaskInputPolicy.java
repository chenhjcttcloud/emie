...
public class SubTaskInputPolicy {
    ...
    public SubTaskInputPolicy(UserService users) {
        this.users = users;
    ...
    public String skillTags(Object raw) {
        ...
        Collection<?> values;
        try {
            values = raw instanceof Collection<?> c
                    ? c
                    : json.readValue(String.valueOf(raw), new TypeReference<List<Object>>() {});
        } catch (Exception e) {
        ...
        List<String> out = values.stream()
                ...
                .map(String::trim)
                .filter(s -> !s.isBlank())
                .map(s -> SecurityUtil.sanitizeText(s, 40))
                .filter(Objects::nonNull)
                .distinct()
                .limit(20)
                .toList();
    ...
    public String collaboratorAllocations(Object raw, String primary) {
        ...
        try {
            rows = raw instanceof Collection<?> c
                    ? json.convertValue(c, new TypeReference<>() {})
                    : json.readValue(String.valueOf(raw), new TypeReference<>() {});
        } catch (Exception e) {
        ...
        for (Map<String, Object> row : rows) {
            String id = SecurityUtil.sanitizeText(String.valueOf(row.getOrDefault("userId", "")), 100);
            int ratio;
            try {
            ...
            if (id == null || id.isBlank() || id.equals(primary) || !seen.add(id))
            ...
            total += ratio;
            out.add(Map.of("userId", id, "name", user.getName(), "ratio", ratio));
        }
