...
public class NotificationWorkflowService {
    ...
    public void notifyUser(
        ...
        if (routedRecipientUserId == null
                || routedRecipientUserId.isBlank()
        ...
        NotificationBundle bundle = requiresNew.execute(status -> {
            NotificationEvent event = outbox.publish(
                    eventType,
                    aggregateType,
                    aggregateId,
                    1,
                    actorUserId,
                    eventType + ":" + aggregateType + ":" + aggregateId + ":" + UUID.randomUUID(),
                    t.content());
            Notification n = notifications.save(Notification.builder()
                    .eventId(event.getId())
                    .recipientUserId(routedRecipientUserId)
                    .category("workflow")
                    .priority(t.priority())
                    ...
                    .build());
            NotificationDelivery inApp = deliveries.save(NotificationDelivery.builder()
        ...
        users.findByUserId(routedRecipientUserId)
                .ifPresent(user -> sendFeishu(bundle.event(), bundle.notification(), user, t, actorUserId));
    }
    ...
    public void notifyUserAfterCommit(
        ...
        Runnable action = () -> {
            try {
                ...
                log.error(
                        "提交后通知创建失败: eventType={}, aggregate={}#{}, recipient={}",
                        eventType,
                        aggregateType,
                        aggregateId,
                        recipientUserId,
    ...
    public void notifyRoleAfterCommit(
            String eventType,
            String role,
            String aggregateType,
            Long aggregateId,
    ...
    private void sendFeishu(
            NotificationEvent event, Notification n, User user, NotificationTemplateService.Template t, String actor) {
        ...
        try {
            d.setExternalMessageId(feishu.sendInteractiveMessage(user.getFeishuOpenId(), t.feishuCardJson()));
    ...
    private void saveFeishuOutcome(
    ...
    private boolean enabled(String key) {
        return configs.findByConfigKey(key)
                ...
                .orElseGet(() -> "notification.feishuEnabled".equals(key)
                        && configs.findByConfigKey("feishu.enabled")
                                .map(SystemConfig::getConfigValue)
                                .map("true"::equalsIgnoreCase)
                                .orElse(false));
    }
    ...
    private void audit(
            NotificationEvent e, Notification n, NotificationDelivery d, String action, String actor, String detail) {
    ...
    private String limit(String s) {
        return s == null ? "未知错误" : s.substring(0, Math.min(s.length(), 1000));
    }
}
