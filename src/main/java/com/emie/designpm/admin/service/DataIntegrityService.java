...
public class DataIntegrityService {
    ...
    public Map<String, Object> scan() {
        ...
        scanBatches(
                fileRecordRepository::findIntegrityFilesAfter,
                record -> knownFiles.add(record.getStoredName()),
                FileRecordRepository.IntegrityFileProjection::getId);
        ...
        scanBatches(
                projectRepository::findIntegrityProjectsAfter,
                project -> {
                    scanJson(
                            "project#" + project.getId() + ".referenceImagesJson",
                            project.getReferenceImagesJson(),
                            knownFiles,
                            referenced,
                            missing,
                            invalidJson,
                            referenceCounts);
                    scanJson(
                            "project#" + project.getId() + ".attachmentsJson",
                            project.getAttachmentsJson(),
                            knownFiles,
                            referenced,
                            missing,
                            invalidJson,
        ...
        scanBatches(
                subTaskRepository::findIntegritySubTasksAfter,
                task -> {
                    scanJson(
                            "sub_task#" + task.getId() + ".referenceImagesJson",
                            task.getReferenceImagesJson(),
                            knownFiles,
                            referenced,
                            missing,
                            invalidJson,
                            referenceCounts);
                    scanJson(
                            "sub_task#" + task.getId() + ".attachmentsJson",
                            task.getAttachmentsJson(),
                            knownFiles,
                            referenced,
                            missing,
                            invalidJson,
        ...
        scanBatches(
                requirementRepository::findIntegrityRequirementsAfter,
                requirement -> {
                    scanJson(
                            "design_requirement#" + requirement.getId() + ".referenceImagesJson",
                            requirement.getReferenceImagesJson(),
                            knownFiles,
                            referenced,
                            missing,
                            invalidJson,
                            referenceCounts);
                    scanJson(
                            "design_requirement#" + requirement.getId() + ".attachmentsJson",
                            requirement.getAttachmentsJson(),
                            knownFiles,
                            referenced,
                            missing,
                            invalidJson,
                            referenceCounts);
                    scanJson(
                            "design_requirement#" + requirement.getId() + ".deliveryReferenceImagesJson",
                            requirement.getDeliveryReferenceImagesJson(),
                            knownFiles,
                            referenced,
                            missing,
                            invalidJson,
                            referenceCounts);
                    scanJson(
                            "design_requirement#" + requirement.getId() + ".deliveryAttachmentsJson",
                            requirement.getDeliveryAttachmentsJson(),
                            knownFiles,
                            referenced,
                            missing,
                            invalidJson,
        ...
        if (materialMarketRepository != null) {
            scanBatches(
                    materialMarketRepository::findIntegrityMaterialsAfter,
                    material -> {
                        scanJson(
                                "material_market#" + material.getId() + ".referenceImagesJson",
                                material.getReferenceImagesJson(),
                                knownFiles,
                                referenced,
                                missing,
                                invalidJson,
                                referenceCounts);
                        scanJson(
                                "material_market#" + material.getId() + ".materialFilesJson",
                                material.getMaterialFilesJson(),
                                knownFiles,
                                referenced,
                                missing,
                                invalidJson,
    ...
    private <T> void scanBatches(
            BatchLoader<T> loader,
            java.util.function.Consumer<T> scanner,
