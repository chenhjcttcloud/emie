...
import com.emie.designpm.points.service.PointProgramOperationsService;import jakarta.servlet.http.HttpServletRequest;import org.springframework.http.*;import org.springframework.web.bind.annotation.*;import java.util.*;
@RestController @RequestMapping("/api/point-program") public class PointProgramOperationsController{
 ...
 private AuthSession session(HttpServletRequest r){AuthSession s=(AuthSession)r.getAttribute("authSession");if(s==null)throw new SecurityException("未登录");return s;}private String str(Map<String,Object>b,String k){return b.get(k)==null?null:String.valueOf(b.get(k));}private Long longValue(Map<String,Object>b,String k){if(b.get(k) instanceof Number n)return n.longValue();throw new IllegalArgumentException(k+"不能为空");}private boolean bool(Map<String,Object>b,String k){if(b.get(k) instanceof Boolean x)return x;throw new IllegalArgumentException(k+"必须为布尔值");}private ResponseEntity<?>call(Action a){try{return ResponseEntity.ok(a.run());}catch(SecurityException e){return ResponseEntity.status(403).body(Map.of("error",e.getMessage()));}catch(IllegalArgumentException e){return ResponseEntity.badRequest().body(Map.of("error",e.getMessage()));}catch(IllegalStateException e){return ResponseEntity.status(409).body(Map.of("error",e.getMessage()));}}interface Action{Object run();}
}
