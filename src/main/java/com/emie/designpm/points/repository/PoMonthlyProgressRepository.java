...
import com.emie.designpm.entity.PoMonthlyProgress;
...
public interface PoMonthlyProgressRepository extends JpaRepository<PoMonthlyProgress, Long> {
    ...
    @Query("select p from PoMonthlyProgress p where p.id=:id")
    Optional<PoMonthlyProgress> findLockedById(@Param("id") Long id);
}
