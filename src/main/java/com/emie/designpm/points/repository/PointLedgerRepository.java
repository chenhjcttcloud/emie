...
public interface PointLedgerRepository extends JpaRepository<PointLedger, Long> {
    ...
    @Query(
            "select coalesce(sum(l.points), 0) from PointLedger l where l.userId = :userId and l.countInPerformance = true")
    double sumPerformancePointsByUserId(@Param("userId") String userId);

    @Query("select l.userId, coalesce(sum(l.points), 0) from PointLedger l " + "where l.countInPerformance = true and "
            + "((:month is not null and l.accountingMonth = :month) or "
            + " (:month is null and (:from is null or (l.createdAt >= :from and l.createdAt < :to)))) "
            + "group by l.userId")
    List<Object[]> sumPerformancePointsByMonth(
