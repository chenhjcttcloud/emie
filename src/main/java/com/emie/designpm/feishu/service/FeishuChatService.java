...
public class FeishuChatService {
    ...
    public FeishuChatService(SystemConfigRepository configs) {
        this.configs = configs;
    }

    public boolean enabled() {
        return !cfg("feishu.ssoAppId").isBlank() && !cfg("feishu.ssoAppSecret").isBlank();
    }
    ...
    public void addMembers(String chatId, Collection<String> openIds) throws Exception {
        List<String> members = openIds == null
                ? List.of()
                : openIds.stream()
                        .filter(Objects::nonNull)
                        .filter(s -> !s.isBlank())
    ...
    private JsonNode request(String method, String path, Object body) throws Exception {
        ...
        if ("POST".equals(method))
            b.POST(HttpRequest.BodyPublishers.ofString(json.writeValueAsString(body == null ? Map.of() : body)));
        else b.DELETE();
        HttpResponse<String> response = http.send(b.build(), HttpResponse.BodyHandlers.ofString());
        JsonNode root = json.readTree(response.body());
        if (response.statusCode() / 100 != 2 || root.path("code").asInt(0) != 0)
            throw new IllegalStateException("飞书群操作失败：" + root.path("msg").asText("HTTP " + response.statusCode()));
        return root;
    }
    ...
    private synchronized String token() throws Exception {
        ...
        String body = json.writeValueAsString(
                Map.of("app_id", cfg("feishu.ssoAppId"), "app_secret", cfg("feishu.ssoAppSecret")));
        HttpRequest request = HttpRequest.newBuilder(URI.create(API + "/auth/v3/tenant_access_token/internal"))
        ...
        if (root.path("code").asInt(0) != 0)
            throw new IllegalStateException("获取飞书访问令牌失败：" + root.path("msg").asText());
        cachedToken = root.path("tenant_access_token").asText();
        tokenExpiresAt = System.currentTimeMillis() + root.path("expire").asLong(7200) * 1000;
        return cachedToken;
    }

    private String cfg(String key) {
        return configs.findByConfigKey(key).map(SystemConfig::getConfigValue).orElse("");
    }
    ...
    private static String required(JsonNode node, String message) {
        if (node == null || node.isMissingNode() || node.asText().isBlank()) throw new IllegalStateException(message);
        return node.asText();
    }
}
