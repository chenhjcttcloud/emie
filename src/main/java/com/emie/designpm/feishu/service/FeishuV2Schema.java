...
final class FeishuV2Schema {
    ...
    record Field(String name, int type, String linkedTable, boolean percentage) {
        static Field text(String name) {
            return new Field(name, TEXT, null, false);
        }

        static Field number(String name) {
            return new Field(name, NUMBER, null, false);
        }

        static Field percentage(String name) {
            return new Field(name, NUMBER, null, true);
        }

        static Field date(String name) {
            return new Field(name, DATE, null, false);
        }

        static Field attachment(String name) {
            return new Field(name, ATTACHMENT, null, false);
        }

        static Field link(String name, String linkedTable) {
            return new Field(name, LINK, linkedTable, false);
        }
    }
        ...
        tables.put(
                ...
                new Table(
                        ...
                        List.of(
                                ...
                                Field.date("计划完成时间"),
                                Field.date("最近更新时间"),
                                Field.link("关联子任务", "task"),
                                Field.text("子任务阶段"),
                                Field.percentage("子任务进度"),
                                Field.text("项目阶段"),
                                Field.percentage("项目总进度"),
                                Field.text("备注"))));
        tables.put(
                "task",
                new Table(
                        ...
                        "系统子任务ID",
                        List.of(
                                Field.link("关联项目", "project"),
                                Field.text("子任务编号"),
                                Field.text("子任务名称"),
                                Field.text("状态"),
                                Field.attachment("参考图片"),
                                Field.attachment("参考附件"),
                                Field.text("超大附件名称"),
                                Field.text("超大附件大小"),
                                Field.text("超大附件链接"),
                                Field.text("子任务负责人"),
                                ...
                                Field.date("计划完成时间"),
                                Field.date("最后更新时间"),
                                Field.attachment("交付图片"),
                                Field.attachment("交付附件"),
                                Field.number("设计师自评分"),
                                Field.number("销售评分"),
                                Field.number("产品企划评分"),
                                Field.number("管理员评分"),
                                Field.number("加权综合"),
                                Field.link("关联评分", "scoring"),
                                Field.text("子任务阶段"),
                                Field.percentage("子任务进度"),
                                Field.text("备注"))));
        tables.put(
    ...
    static Map<String, Table> backupTables() {
        ...
        primaryTables().forEach((key, table) -> {
            ...
            result.put(
                    key + "Backup",
                    new Table(key + "Backup", table.name() + "_backup", table.identity(), List.copyOf(fields)));
        });
        return result;
    }
