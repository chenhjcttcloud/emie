...
class PointManualAdjustmentServiceTest {
    ...
    void boundaryPointsAreAccepted() {
        ...
        assertEquals(
                -100000, service.adjust("designer-1", -100000, "下限扣分", admin()).getPoints());
    }
    ...
    void salesAndSupplyChainTargetsAreRejected() {
        ...
        IllegalArgumentException e =
                assertThrows(IllegalArgumentException.class, () -> service.adjust("sales-1", 10, "备注", admin()));
        assertEquals("只能为设计师调账", e.getMessage());
        IllegalArgumentException supply =
                assertThrows(IllegalArgumentException.class, () -> service.adjust("supply-1", 10, "备注", admin()));
    ...
    void plannerAndAdminTargetsAreRejected() {
        ...
        assertThrows(IllegalArgumentException.class, () -> service.adjust("planner-1", 10, "备注", admin()));
        assertThrows(IllegalArgumentException.class, () -> service.adjust("admin-1", 10, "备注", admin()));
        verify(adjustments, never()).save(any());
    }
