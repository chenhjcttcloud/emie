...
class NotificationTemplateServiceTest {
    ...
    void redeliveryTemplateKeepsDeliveryCountAndRejectionReason() throws Exception {
        ...
        assertTrue(new ObjectMapper()
                .readTree(result.feishuCardJson())
                .path("body")
                .path("elements")
                .isArray());
    }
    ...
    void materialMarketPlannerPendingUsesDedicatedCopy() {
        var result = service.render(
                "MATERIAL_MARKET_PLANNER_PENDING",
                Map.of("projectName", "桌面收纳灯", "actorName", "销售小李", "projectLink", "/?projectId=109"));
    ...
    void legacyProjectDeepLinkIsConvertedToSpaProjectLink() {
        var result = service.render(
                "MATERIAL_MARKET_PLANNER_PENDING", Map.of("projectName", "项目 A", "projectLink", "/projects/114"));
    ...
    void configuredTemplateOverridesDefaultAndResolvesVariables() {
        when(configs.findByConfigKey("notification.template.TASK_ASSIGNED.title"))
                .thenReturn(Optional.of(
                        SystemConfig.builder().configValue("请处理：{{taskName}}").build()));
        when(configs.findByConfigKey("notification.template.TASK_ASSIGNED.content"))
                .thenReturn(Optional.of(SystemConfig.builder()
                        .configValue("{{actorName}}派发了{{projectName}}，截止{{deadline}}")
                        .build()));
        var result = service.render(
                "TASK_ASSIGNED",
                Map.of("projectName", "环球杯子", "taskName", "小黄人包装", "actorName", "小李", "deadline", "2026-07-20"));
